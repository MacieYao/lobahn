...
class Candidate extends CI_Controller {
    ...
    public function get_saved_candidates() {
		...
		foreach ($results as $row) {
            ...
            // NB location no longer stored in users table so removed 20150602
    ...
    public function get_details() {
        ...
        $query = "
            SELECT
                ...
                u.current_salary,
                GROUP_CONCAT(
                    IF(dl.name_en IS NULL, '', dl.name_en) SEPARATOR '##') AS locations
            FROM
                (
                    SELECT *
                    FROM ".TABLE_USERS."
    ...
    // NB removes a job feed for an employer user, so sets job to inactive
	...
	public function get_remove_candidates() {
		...
		foreach ($results as $row) {
            ...
            // NB location info no longer held in users table removed by 20150602
            // Prepare the location information of the candidate
            // if ($row['location'] == "") {
                // $row['location'] = array();
            // } else {
