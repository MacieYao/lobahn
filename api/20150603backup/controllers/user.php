...
class User extends CI_Controller {
    ...
    public function login() {
        ...
        // NB users can have multiple messages since they last logged in and multiple CVs
         $query = "
            SELECT id, message
        ...
        // 20150602 TR changed back. Users can have multiple CVs and messages
		/*$query = "
            SELECT id, message
            FROM ".TABLE_USERS_MESSAGES."
    ...
    public function update_user_profile() {
        ...
        $query = "
            ...
            (user_id, location_id, location)
            VALUES
            (?, ?, ?)
        ";
        $data = array($user['id'], $location_id, $location_name);
        $this->db->query($query, $data);
...
// TR addition to update just the user's links 20150331
    ...
    //get the list of jobs the user has hidden 
    public function get_hidden_jobs() {
        $token = $this->input->get_post('token');
        ...
        $query = "
            ...
            FROM
                ...
                (
                    ...
                    ON xjobs.id = hiddenjobs.job_id
            ORDER BY hiddenjobs.timestamp DESC
        ";
    ...
    public function remove_hidden_job() {
        $token = $this->input->get_post('token');
        $job_id = $this->input->get_post('job_id');
                ...
                (
                    ...
                    FROM
                        (
                            ...
                            LEFT OUTER JOIN jobs_locations xlcodes
                            ON xlocs.id = xlcodes.location_id
                                LEFT OUTER JOIN jobs jobs
                                ON xlcodes.job_id = jobs.id
    ...
    public function get_user_data() {
        ...
        // Get messages for job applications saved by the user
        // NB can be multiple messages since last check
         $query = "
            SELECT id, message
            FROM ".TABLE_USERS_MESSAGES."
            WHERE user_id = ?
...
*/        
		...
		// Get cv for job applications saved by the user
        // NB can be multiple CVs per user
        $query = "
            SELECT id, cv
            FROM users_cv
