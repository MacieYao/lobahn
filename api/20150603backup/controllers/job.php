...
class Job extends CI_Controller {
            ...
            $query = "
                ...
                FROM
                    ...
                    (
                        ...
                        AND jobs.salary_min >= ".$target_salary."
                        ".$industries_filter."
                        ".$company_filter."
                    ) xjobs
    ...
    public function get_num_of_updates(){
        ...
        // debug database action
        log_message('debug', $this->db->last_query());
        log_message('debug', print_r($results_jobs, true));
        ...
        // debug database action
        log_message('debug', $this->db->last_query());
        log_message('debug', print_r($results_candidates, true));
        ...
        $query = "
            ...
            FROM
                (
                    ...
                    ON nj.job_id = j.id
                        LEFT OUTER JOIN jobs_locations loc
                        ON j.id = loc.job_id
                            LEFT OUTER JOIN data_locations dloc
                            ON loc.location_id = dloc.id
            WHERE
    ...
    public function get_posted_jobs() {
        ...
        $query = "
			...
			FROM
                jobs j
                LEFT OUTER JOIN jobs_locations jloc
                    ON j.id = jloc.job_id
                    LEFT OUTER JOIN data_locations d
                        ON jloc.location_id = d.id
            WHERE
                j.user_id = ?
            AND j.status = 1
            GROUP BY j.id
